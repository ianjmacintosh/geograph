...
import { generateComputerGuess, calculateDistance, calculateBonusPoints } from '../utils/game';
    ...
    const computerGuessTimer = setTimeout(() => {
      setCurrentRound(prevRoundState => {
        ...
        const newComputerGuesses: Guess[] = stillNeedToGuess.map(player => {
          ...
          const guessDetails = generateComputerGuess(cityToGuess, player.accuracy || 0.5);
          const distance = calculateDistance(cityToGuess.lat, cityToGuess.lng, guessDetails.lat, guessDetails.lng);
