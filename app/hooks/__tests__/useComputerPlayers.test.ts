...
const computerPlayer1: Player = { id: 'cpu1', name: 'CPU 1', isComputer: true, score: 0, accuracy: 0.7 };
const computerPlayer2: Player = { id: 'cpu2', name: 'CPU 2', isComputer: true, score: 0, accuracy: 0.5 };
...
const mockCurrentGameTwoComputers: Game = {
  ...
  rounds: [],
  status: 'playing',
  settings: { maxPlayers: 3, roundTimeLimit: 30000, totalRounds: 5, cityDifficulty: 'easy' },
  createdAt: Date.now(),
};
...
describe('useComputerPlayers', () => {
  ...
  it('should make multiple computer players guess', () => {
    ...
    expect(newRound.guesses.some((g: Guess) => g.playerId === computerPlayer1.id)).toBe(true);
    expect(newRound.guesses.some((g: Guess) => g.playerId === computerPlayer2.id)).toBe(true);
  });
  ...
  it('should call onComputerGuessesCompleteRound if computer guesses complete the round', () => {
     renderHook(() => useComputerPlayers({
      ...
      currentRound: { ...mockInitialRound, guesses: [{playerId: humanPlayer.id, lat:0,lng:0,distance:0,bonusPoints:0,placement:0,placementPoints:0,timestamp:0,totalPoints:0}] }, // Human has guessed
      isHumanPlayerTurnOrResultsShown: false,
      setCurrentRound: mockSetCurrentRound,
      onComputerGuessesCompleteRound: mockOnComputerGuessesCompleteRound,
    }));
    ...
    const functionalUpdate = mockSetCurrentRound.mock.calls[0][0];
    act(() => {
        const roundBeforeUpdate = { ...mockInitialRound, guesses: [{playerId: humanPlayer.id, lat:0,lng:0,distance:0,bonusPoints:0,placement:0,placementPoints:0,timestamp:0,totalPoints:0}] };
        const updatedRound = functionalUpdate(roundBeforeUpdate);
         // If this was the last guess, the callback should be called
        if (updatedRound.guesses.length === mockCurrentGameOneComputer.players.length) {
  ...
  it('should clear timeout on unmount', () => {
    ...
    const { unmount } = renderHook(() => useComputerPlayers({
      currentGame: mockCurrentGameOneComputer,
      currentRound: mockInitialRound,
      isHumanPlayerTurnOrResultsShown: false,
      setCurrentRound: mockSetCurrentRound,
  ...
  it('should handle stale closure for currentRound correctly in setCurrentRound callback', () => {
    const { rerender } = renderHook(
      (props) => useComputerPlayers(props),
      { initialProps: {
          currentGame: mockCurrentGameOneComputer,
