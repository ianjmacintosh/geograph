...
vi.mock('../../contexts/GameContext', async (importOriginal) => {
 const actual = await importOriginal<typeof import('../../contexts/GameContext')>();
  return {
    ...actual,
    useGame: () => ({ // Default mock for useGame
      currentGame: null,
      finishGame: vi.fn(),
      clearGame: vi.fn(),
    }),
...
const mockCity1: City = { id: 'city1', name: 'City One', country: 'C1', lat: 0, lng: 0, population: 1, difficulty: 'easy' };
const mockCity2: City = { id: 'city2', name: 'City Two', country: 'C2', lat: 1, lng: 1, population: 1, difficulty: 'easy' };

const humanPlayer: Player = { id: 'p1', name: 'Human', isComputer: false, score: 0 };
const mockBaseGame: Game = {
  ...
  settings: { maxPlayers: 1, roundTimeLimit: 30000, totalRounds: 2, cityDifficulty: 'easy' },
  createdAt: Date.now(),
};
...
describe('useRoundManagement', () => {
  ...
  const setupHook = async (gameProps: Partial<Game> = {}, initialRoundNumber = 1) => {
    ...
    vi.mocked(citiesMock.getRandomCityByDifficulty).mockImplementation(mockGetRandomCity);
    mockGetRandomCity.mockReturnValueOnce(mockCity1).mockReturnValueOnce(mockCity2);
  ...
  it('handleNextRound should advance to next round', async () => {
    ...
    act(() => {
      ...
      result.current.setCurrentRound({ ...firstRound, completed: true, guesses: [{playerId: 'p1', lat:0,lng:0,distance:0,bonusPoints:0,placement:1,placementPoints:1,timestamp:0,totalPoints:1}] });
    });
  ...
  it('handleNextRound should call handleGameEnd if last round is completed', async () => {
    const gameSettings = { settings: { ...mockBaseGame.settings, totalRounds: 1 } }; // Only 1 round
  ...
  it('handleGameEnd should calculate final results and navigate', async () => {
    ...
    const testRound: GameRound = {
        id: 'r1', city: mockCity1, guesses: [
            { playerId: 'p1', lat: 0, lng: 0, distance: 10, bonusPoints: 5, totalPoints: 15, placement:1, placementPoints: 10, timestamp: Date.now() }
        ], completed: true, startTime: Date.now(), endTime: Date.now()
    };
    act(() => {
  ...
  it('should use updateRoundWithPlacements when adding current round to completedRounds in handleNextRound', async () => {
    ...
    const roundWithGuesses: GameRound = { ...firstRound, guesses: [{playerId: 'p1', lat:0,lng:0,distance:0,bonusPoints:0,placement:0,placementPoints:0,timestamp:0,totalPoints:0}] };
    act(() => {
    ...
    expect(mockUpdateRoundWithPlacements).toHaveBeenCalledWith(expect.objectContaining({ id: firstRound.id }));
    expect(result.current.completedRounds.length).toBe(1);
    expect(result.current.completedRounds[0].id).toBe(firstRound.id);
  ...
  it('should correctly include a non-completed current round in handleGameEnd if it has guesses', async () => {
    const { result } = await setupHook({ settings: { ...mockBaseGame.settings, totalRounds: 1 }}); // Game ends after 1 round

    const currentRoundInProgress = result.current.currentRound!;
    const roundWithGuessNotCompleted: GameRound = {
        ...currentRoundInProgress,
        guesses: [{playerId: 'p1', lat:0,lng:0,distance:50,bonusPoints:2,placement:0,placementPoints:0,timestamp:0,totalPoints:0}], // No total points yet
    ...
    expect(mockUpdateRoundWithPlacements).toHaveBeenCalledWith(expect.objectContaining({ id: currentRoundInProgress.id }));
    expect(mockFinishGameContext).toHaveBeenCalledTimes(1);
    const finalResults = mockFinishGameContext.mock.calls[0][0] as FinalResults;
