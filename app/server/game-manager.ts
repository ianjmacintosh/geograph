...
export class GameManager {
  ...
  startGame(gameId: string, playerId: string): GameResult {
    ...
    // Start first round
    const firstRound = this.createNewRound(gameId, game.settings.cityDifficulty, []);
  ...
  addComputerPlayers(gameId: string, count: number): GameResult {
    ...
    if (game.status !== 'waiting') {
      return { success: false, error: 'Cannot add computer players after game has started' };
    }
    ...
    const actualCount = Math.min(count, availableNames.length, game.settings.maxPlayers - game.players.length);
  ...
  makeGuess(gameId: string, playerId: string, lat: number, lng: number): GameResult {
    const game = this.db.getGameById(gameId);
    ...
    const distance = calculateDistance(lat, lng, currentRound.city.lat, currentRound.city.lng);
    ...
    const humanGuesses = currentRound.guesses.filter(g => 
  ...
  private async endRound(gameId: string, roundId: string) {
    ...
    const placements = calculatePlacementPoints(guessesWithDistance, game.players.length);
    
    // Update each guess with placement points
    for (const placement of placements) {
      const guess = round.guesses.find(g => g.playerId === placement.playerId);
      if (guess) {
  ...
  nextRound(gameId: string, playerId: string): GameResult {
    ...
    const nextRound = this.createNewRound(gameId, game.settings.cityDifficulty, usedCityIds);
  ...
  removePlayer(gameId: string, playerId: string): GameResult {
    ...
    if (game.hostId === playerId && game.status === 'waiting') {
      const remainingPlayers = game.players.filter(p => p.id !== playerId && !p.isComputer);
