...
vi.mock('../utils/game', () => ({
  calculateDistance: vi.fn((lat1: number, lng1: number, lat2: number, lng2: number) => {
    // Human perfect guess
    if (lat1 === 40.7128 && lng1 === -74.0060) return 0;
    // Computer 1 guess
  ...
  calculatePlacementPoints: vi.fn((guesses: Array<{playerId: string, distance: number}>, totalPlayers: number) => {
    console.log(`calculatePlacementPoints: ${guesses.length} guesses for ${totalPlayers} players`);
...
describe('Computer Guess Integration', () => {
  ...
  it('should add computer guesses to the round state', async () => {
    ...
    if (finalGuessCount !== '3') {
      ...
      throw new Error(`Computer guesses not integrated! Expected 3 guesses, got ${finalGuessCount}`);
    }
  ...
  it('should have consistent state updates for computer guesses', async () => {
    let stateSnapshots: Array<{time: number, guessCount: number, event: string}> = [];
    
    const captureState = (event: string) => {
      try {
        const guessCount = parseInt(screen.getByTestId('guess-count').textContent || '0');
        stateSnapshots.push({
          time: Date.now(),
          guessCount,
          event
    ...
    if (finalSnapshot.guessCount !== 3) {
      ...
      console.error('Computer guesses are not being properly integrated into component state');
      throw new Error(`Expected 3 final guesses, got ${finalSnapshot.guessCount}`);
    }
    ...
    for (let i = 1; i < stateSnapshots.length; i++) {
      ...
      if (curr.guessCount < prev.guessCount) {
        console.error(`❌ State regression detected: ${prev.event}(${prev.guessCount}) -> ${curr.event}(${curr.guessCount})`);
        throw new Error(`Guess count decreased from ${prev.guessCount} to ${curr.guessCount}`);
      }
    }
