      ...
      <div data-testid="guess-details">
        {guesses.map((guess: any, index: number) => (
          <div key={index} data-testid={`guess-detail-${index}`}>
            Player: {guess.playerName} | Type: {guess.isComputer ? 'Computer' : 'Human'} | 
            Coords: {guess.lat.toFixed(4)}, {guess.lng.toFixed(4)}
          </div>
        ))}
      </div>
    </div>
...
describe('Computer Guess Persistence Bug', () => {
  ...
  it('should preserve computer guesses when human makes a guess', async () => {
    ...
    const guessesAfterComputerGuesses = screen.getByTestId('total-guesses').textContent;
    console.log('Guesses after computer auto-guesses:', guessesAfterComputerGuesses);
    ...
    if (guessesAfterComputerGuesses !== '0') {
      console.log('✅ Computers made initial guesses:', guessesAfterComputerGuesses);
      ...
      const guessesAfterHumanGuess = screen.getByTestId('total-guesses').textContent;
      ...
      if (parseInt(guessesAfterHumanGuess || '0') < parseInt(guessesAfterComputerGuesses || '0')) {
        console.error('❌ BUG CONFIRMED: Computer guesses DISAPPEARED after human guess!');
        console.error(`Before human guess: ${guessesAfterComputerGuesses} guesses`);
        ...
        throw new Error(`Computer guess persistence bug! Guesses went from ${guessesAfterComputerGuesses} to ${guessesAfterHumanGuess}`);
      }
      ...
      console.log('⚠️  Computers did not make initial guesses - different issue');
      ...
      if (finalCount === '1') {
        console.error('❌ BUG CONFIRMED: Computers never made guesses after human!');
