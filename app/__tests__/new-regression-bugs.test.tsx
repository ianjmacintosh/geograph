...
describe('New Regression Bugs', () => {
  ...
  it('should prevent multiple guesses from the same player - EXPECTED TO FAIL', async () => {
    ...
    if (humanGuesses.length > 1) {
      throw new Error(`Multiple guessing bug detected! Human player made ${humanGuesses.length} guesses, should only be 1`);
    }

    if (afterSecondClick !== '1' && !afterSecondClick?.startsWith('1')) {
      throw new Error(`Multiple guessing bug detected! Total guesses increased from 1 to ${afterSecondClick} after second click`);
    }
  ...
  it('should hide other players guesses until human player guesses - EXPECTED TO FAIL', async () => {
    ...
    console.log('Visible guesses before human guess:', visibleGuessesBefore.length);
    ...
    const computerGuessesVisible = visibleGuessesBefore.filter(el => 
      el.textContent?.includes('Computer1') || el.textContent?.includes('Computer2')
    );

    if (computerGuessesVisible.length > 0) {
      console.log('❌ BUG: Computer guesses are visible before human player guesses!');
      throw new Error(`Guess visibility bug detected! ${computerGuessesVisible.length} computer guesses visible before human guess`);
    }
    ...
    console.log('Visible guesses after human guess:', visibleGuessesAfter.length);
  ...
  it('should end round after all players guess OR timer expires - EXPECTED TO FAIL', async () => {
    ...
    const roundEndedWithAllGuesses = await Promise.race([
      ...
      }, { timeout: 10000 }).then(() => 'results').catch(() => null),
      ...
      }, { timeout: 10000 }).then(() => 'timer').catch(() => null),
    ...
    if (roundEndedWithAllGuesses === 'timeout') {
      throw new Error('Round ending bug detected! Round did not end within reasonable time');
    }
    ...
    if (!roundResults) {
      throw new Error('Round ending bug detected! No round results shown after round should have ended');
    }
  ...
  it('should progress through multiple rounds correctly', async () => {
    ...
    try {
      ...
      const currentText = screen.getByText(/Click on the map to guess where|✅ Guess submitted!/);
