...
describe('Computer Players First Round Bug', () => {
  ...
  beforeEach(() => {
    ...
    mockGame = {
      ...
      players: [
        ...
        { id: 'player4', name: 'Computer3', isComputer: true, score: 0, accuracy: 0.7 }
      ],
      rounds: [],
  ...
  it('should have computer players make guesses in the first round', async () => {
    ...
    if (finalGuessCount !== '4') {
      console.error('❌ BUG CONFIRMED: Computer players did not all make guesses in first round!');
      ...
      const expectedPlayers = ['Human Player', 'Computer1', 'Computer2', 'Computer3'];
      const missingPlayers = expectedPlayers.filter(name => !playerNames.includes(name));
      console.error('Players who did NOT make guesses:', missingPlayers);
      
      throw new Error(`Computer guessing bug confirmed! Expected 4 guesses, got ${finalGuessCount}. Missing players: ${missingPlayers.join(', ')}`);
    }
  ...
  it('should show who has guessed in the scoreboard', async () => {
    ...
    console.log('Checking initial scoreboard state - all players should show "Waiting"');
