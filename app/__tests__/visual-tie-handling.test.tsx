...
const createTieGame = () => ({
  ...
  players: [
    ...
    { id: 'computer-charlie', name: 'Charlie', isComputer: true, score: 0, accuracy: 0.5 },
  ],
  rounds: [
    {
      id: 'round-1',
      city: { id: 'paris', name: 'Paris', country: 'France', lat: 48.8566, lng: 2.3522, population: 2161000, difficulty: 'easy' as const },
      guesses: [
        { playerId: 'player-alice', lat: 48.8566, lng: 2.3522, distance: 100, placementPoints: 3, bonusPoints: 5, totalPoints: 8, placement: 1, timestamp: Date.now() },
        { playerId: 'player-bob', lat: 48.8566, lng: 2.3522, distance: 100, placementPoints: 3, bonusPoints: 5, totalPoints: 8, placement: 1, timestamp: Date.now() },
        { playerId: 'computer-charlie', lat: 48.8566, lng: 2.3522, distance: 200, placementPoints: 1, bonusPoints: 2, totalPoints: 3, placement: 3, timestamp: Date.now() },
      ],
      completed: true,
      startTime: Date.now(),
      endTime: Date.now(),
  ...
  finalResults: {
    playerScores: [
      { playerId: 'player-alice', playerName: 'Alice', isComputer: false, totalScore: 15, finalPlacement: 1 },
      { playerId: 'player-bob', playerName: 'Bob', isComputer: false, totalScore: 15, finalPlacement: 1 }, // Tied with Alice
      { playerId: 'computer-charlie', playerName: 'Charlie', isComputer: true, totalScore: 10, finalPlacement: 3 }, // 3rd place (2nd skipped)
    ],
...
describe('Visual Tie Handling Test', () => {
  ...
  it('should visually demonstrate tie handling in game results', async () => {
    ...
    console.log('This test demonstrates how ties are handled in the game interface\n');
    ...
    const MockGameProvider = ({ children }: { children: React.ReactNode }) => {
      ...
      return (
        <GameProvider value={mockContextValue}>
          {children}
        </GameProvider>
      );
    };
    ...
    console.log('📊 Placement indicators found:', placementElements.map(el => el.textContent));
    ...
    console.log('🥉 Found 1 player in 3rd place (2nd place skipped due to tie)');

    // Test the actual tie handling logic
    const playerScores = [
      { playerId: 'alice', playerName: 'Alice', isComputer: false, totalScore: 15, finalPlacement: 0 },
      { playerId: 'bob', playerName: 'Bob', isComputer: false, totalScore: 15, finalPlacement: 0 },
      { playerId: 'charlie', playerName: 'Charlie', isComputer: true, totalScore: 10, finalPlacement: 0 },
    ];

    // Use the real calculateFinalPlacements function (not mocked)
    const { calculateFinalPlacements: realCalculateFinalPlacements } = await vi.importActual('../utils/game') as any;
    const results = realCalculateFinalPlacements(playerScores);

    console.log('🔍 Testing actual tie handling logic:');
    console.log('Input scores:', playerScores.map(p => `${p.playerName}: ${p.totalScore}`));
    console.log('Output placements:', results.map(p => `${p.playerName}: #${p.finalPlacement} (${p.totalScore} points)`));
  ...
  it('should demonstrate round-level tie handling', async () => {
    ...
    const { calculatePlacementPoints: realCalculatePlacementPoints } = await vi.importActual('../utils/game') as any;
    const roundResults = realCalculatePlacementPoints(roundGuesses, 3);

    console.log('🔍 Round tie handling:');
    console.log('Input distances:', roundGuesses.map(g => `${g.playerId}: ${g.distance}km`));
    console.log('Output placements:', roundResults.map(r => `${r.playerId}: #${r.placement} (${r.placementPoints} points)`));
