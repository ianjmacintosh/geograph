...
vi.mock('../utils/game', () => ({
  calculateDistance: (lat1: number, lng1: number, lat2: number, lng2: number) => {
    ...
    if (lat1 === 40.7128 && lng1 === -74.0060) return 0; // Perfect guess
...
describe('Flickering Detection Test', () => {
  ...
  it('should detect score flickering during computer guess processing', async () => {
    ...
    const mockRound = {
      id: 'round1',
      city: { id: '1', name: 'Test City', country: 'Test Country', lat: 40.7128, lng: -74.0060, population: 1000000, difficulty: 'easy' as const },
      guesses: [
        // Initial state: guesses created but totalPoints not yet calculated
        { playerId: 'player1', lat: 40.7128, lng: -74.0060, distance: 0, placementPoints: 0, bonusPoints: 5, totalPoints: 0, placement: 0, timestamp: Date.now() },
        { playerId: 'player2', lat: 41.0, lng: -73.0, distance: 150, placementPoints: 0, bonusPoints: 2, totalPoints: 0, placement: 0, timestamp: Date.now() },
        { playerId: 'player3', lat: 39.0, lng: -75.0, distance: 300, placementPoints: 0, bonusPoints: 2, totalPoints: 0, placement: 0, timestamp: Date.now() }
      ],
      completed: false,
      startTime: Date.now()
    };
    ...
    const mockRoundWithCalculatedPlacements = {
      ...mockRound,
      guesses: [
        { playerId: 'player1', lat: 40.7128, lng: -74.0060, distance: 0, placementPoints: 3, bonusPoints: 5, totalPoints: 8, placement: 1, timestamp: Date.now() },
        { playerId: 'player2', lat: 41.0, lng: -73.0, distance: 150, placementPoints: 2, bonusPoints: 2, totalPoints: 4, placement: 2, timestamp: Date.now() },
        { playerId: 'player3', lat: 39.0, lng: -75.0, distance: 300, placementPoints: 1, bonusPoints: 2, totalPoints: 3, placement: 3, timestamp: Date.now() }
      ]
    };
    ...
    const oldGetPlayerScores = (currentRound: any) => {
      return players.map(player => {
        let totalScore = 0;
        if (currentRound) {
          const playerGuess = currentRound.guesses.find((g: any) => g.playerId === player.id);
          if (playerGuess && playerGuess.totalPoints > 0) { // BUG: hides scores when totalPoints is temporarily 0
    ...
    const newGetPlayerScores = (currentRound: any) => {
      return players.map(player => {
        ...
        if (currentRound) {
          const playerGuess = currentRound.guesses.find((g: any) => g.playerId === player.id);
          if (playerGuess && playerGuess.placementPoints > 0) { // FIX: shows scores when placements are calculated
    ...
    const intermediateRound = {
      ...mockRound,
      guesses: [
        { playerId: 'player1', lat: 40.7128, lng: -74.0060, distance: 0, placementPoints: 3, bonusPoints: 5, totalPoints: 0, placement: 1, timestamp: Date.now() }, // totalPoints temporarily 0
        { playerId: 'player2', lat: 41.0, lng: -73.0, distance: 150, placementPoints: 2, bonusPoints: 2, totalPoints: 0, placement: 2, timestamp: Date.now() }, // totalPoints temporarily 0
        { playerId: 'player3', lat: 39.0, lng: -75.0, distance: 300, placementPoints: 1, bonusPoints: 2, totalPoints: 0, placement: 3, timestamp: Date.now() } // totalPoints temporarily 0
      ]
    };
    ...
    const oldScoresCalculated = oldGetPlayerScores(mockRoundWithCalculatedPlacements);
    const newScoresCalculated = newGetPlayerScores(mockRoundWithCalculatedPlacements);
    
    // Both should show proper scores when fully calculated
    expect(oldScoresCalculated.find(p => p.id === 'player1')?.totalScore).toBe(8);
    expect(oldScoresCalculated.find(p => p.id === 'player2')?.totalScore).toBe(4);
    expect(oldScoresCalculated.find(p => p.id === 'player3')?.totalScore).toBe(3);
    
    expect(newScoresCalculated.find(p => p.id === 'player1')?.totalScore).toBe(8);
    expect(newScoresCalculated.find(p => p.id === 'player2')?.totalScore).toBe(4);
    expect(newScoresCalculated.find(p => p.id === 'player3')?.totalScore).toBe(3);
    ...
    const bugScenarioRound = {
      ...
      guesses: [
        { playerId: 'player1', lat: 40.7128, lng: -74.0060, distance: 0, placementPoints: 3, bonusPoints: 5, totalPoints: 8, placement: 1, timestamp: Date.now() },
        { playerId: 'player2', lat: 41.0, lng: -73.0, distance: 150, placementPoints: 2, bonusPoints: 2, totalPoints: 0, placement: 2, timestamp: Date.now() }, // totalPoints reset during update
        { playerId: 'player3', lat: 39.0, lng: -75.0, distance: 300, placementPoints: 1, bonusPoints: 2, totalPoints: 0, placement: 3, timestamp: Date.now() } // totalPoints reset during update
      ]
    };
    ...
    expect(oldScoresBugScenario.find(p => p.id === 'player1')?.totalScore).toBe(8);
    expect(oldScoresBugScenario.find(p => p.id === 'player2')?.totalScore).toBe(0); // BUG: should be 4
    expect(oldScoresBugScenario.find(p => p.id === 'player3')?.totalScore).toBe(0); // BUG: should be 3

    // New logic: all players get scores based on placement calculation (fallback calculation)
    expect(newScoresBugScenario.find(p => p.id === 'player1')?.totalScore).toBe(8);
    expect(newScoresBugScenario.find(p => p.id === 'player2')?.totalScore).toBe(4); // FIXED: 2 + 2 = 4 (calculated from placement + bonus)
    expect(newScoresBugScenario.find(p => p.id === 'player3')?.totalScore).toBe(3); // FIXED: 1 + 2 = 3 (calculated from placement + bonus)

    console.log('✅ Scoring logic test passed - fix prevents flickering when scores are properly calculated');
  });
  ...
  it('should verify scores become visible and stay visible after calculations', () => {
    ...
    const stateSequence = [
      ...
      {
        guesses: [
          { playerId: 'player1', placementPoints: 0, bonusPoints: 5, totalPoints: 0, placement: 0 },
          { playerId: 'player2', placementPoints: 0, bonusPoints: 2, totalPoints: 0, placement: 0 },
          { playerId: 'player3', placementPoints: 0, bonusPoints: 2, totalPoints: 0, placement: 0 }
        ]
      },
      // 2. Placement points calculated, but totalPoints not yet updated (the flickering scenario)
      {
        guesses: [
          { playerId: 'player1', placementPoints: 3, bonusPoints: 5, totalPoints: 0, placement: 1 },
          { playerId: 'player2', placementPoints: 2, bonusPoints: 2, totalPoints: 0, placement: 2 },
          { playerId: 'player3', placementPoints: 1, bonusPoints: 2, totalPoints: 0, placement: 3 }
        ]
      },
      // 3. Some totalPoints updated but not others (partial state)
      {
        guesses: [
          { playerId: 'player1', placementPoints: 3, bonusPoints: 5, totalPoints: 8, placement: 1 },
          { playerId: 'player2', placementPoints: 2, bonusPoints: 2, totalPoints: 0, placement: 2 }, // Still 0
          { playerId: 'player3', placementPoints: 1, bonusPoints: 2, totalPoints: 0, placement: 3 }  // Still 0
        ]
      },
      // 4. Fully calculated state
      {
        guesses: [
          { playerId: 'player1', placementPoints: 3, bonusPoints: 5, totalPoints: 8, placement: 1 },
          { playerId: 'player2', placementPoints: 2, bonusPoints: 2, totalPoints: 4, placement: 2 },
          { playerId: 'player3', placementPoints: 1, bonusPoints: 2, totalPoints: 3, placement: 3 }
        ]
      }
    ];
    ...
    const getPlayerScoresFixed = (round: any) => {
      return players.map(player => {
        let totalScore = 0;
        if (round) {
          const playerGuess = round.guesses.find((g: any) => g.playerId === player.id);
    ...
    const getPlayerScoresBuggy = (round: any) => {
      return players.map(player => {
        ...
        if (round) {
          const playerGuess = round.guesses.find((g: any) => g.playerId === player.id);
          if (playerGuess && playerGuess.totalPoints > 0) { // BUG: only shows when totalPoints > 0
            totalScore += playerGuess.totalPoints;
          }
        }
        return { ...player, totalScore };
    ...
    stateSequence.forEach((state, stateIndex) => {
      ...
      players.forEach((player, playerIndex) => {
        const fixedScore = fixedScores.find(p => p.id === player.id)?.totalScore || 0;
        const buggyScore = buggyScores.find(p => p.id === player.id)?.totalScore || 0;
      ...
      console.log('Fixed scores:', fixedScores.map(p => ({ [p.id]: p.totalScore })));
      console.log('Buggy scores:', buggyScores.map(p => ({ [p.id]: p.totalScore })));
    });
    ...
    players.forEach((player, playerIndex) => {
      ...
      for (let i = 1; i < fixedHistory.length; i++) {
        if (fixedHistory[i-1] > 0 && fixedHistory[i] < fixedHistory[i-1]) {
          throw new Error(`Fixed version: Score decreased for ${player.id}: ${fixedHistory[i-1]} -> ${fixedHistory[i]}`);
        }
      }
      ...
      for (let i = 1; i < buggyHistory.length; i++) {
        if (buggyHistory[i-1] > 0 && buggyHistory[i] === 0) {
          ...
          console.log(`Buggy version flickering detected for ${player.id}: ${buggyHistory[i-1]} -> ${buggyHistory[i]}`);
        }
      }

      // For computer players, we expect to see flickering in the buggy version
      if (player.isComputer && !foundFlickering) {
        console.warn(`Expected flickering for computer player ${player.id} but didn't find it in buggy version`);
      }
    });

    // Final verification: fixed version should show proper final scores
    const finalFixedScores = getPlayerScoresFixed(stateSequence[stateSequence.length - 1]);
    expect(finalFixedScores.find(p => p.id === 'player1')?.totalScore).toBe(8);
    expect(finalFixedScores.find(p => p.id === 'player2')?.totalScore).toBe(4);
    expect(finalFixedScores.find(p => p.id === 'player3')?.totalScore).toBe(3);

    console.log('✅ Scores remained stable after becoming non-zero - no flickering detected with fix');
  });
});