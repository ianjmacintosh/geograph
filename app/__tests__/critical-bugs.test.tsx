  ...
  WorldMap: ({ onMapClick, guesses, showTarget }: any) => {
    ...
    return (
      <div data-testid="world-map">
        <button
          onClick={() => {
            ...
            if (onMapClick) {
              ...
              const positions = [
                [40.7128, -74.0060], // First click
                [41.0, -73.0],       // Second click  
                [39.0, -75.0],       // Third click
                [42.0, -72.0],       // Fourth click
                [38.0, -76.0]        // Fifth click
              ];
              const pos = positions[clickCount - 1] || [40.7128, -74.0060];
              onMapClick(pos[0], pos[1]);
            } else {
              console.log('onMapClick is null/undefined - clicks should be blocked');
            }
          }}
        ...
        <div data-testid="click-enabled">{onMapClick ? 'enabled' : 'disabled'}</div>
...
describe('Critical Bugs', () => {
  ...
  it('should end round immediately when all players have guessed - EXPECTED TO FAIL', async () => {
    ...
    const afterComputerGuesses = screen.getByTestId('total-guesses').textContent;
    ...
    console.log('All players have guessed (3/3) - round should end immediately');
    ...
    const roundEndedQuickly = await Promise.race([
      ...
      new Promise(resolve => setTimeout(() => {
        console.log('❌ 3 second timeout - round should have ended immediately when all players guessed');
        resolve(false);
      }, 3000))
    ]);

    if (!roundEndedQuickly) {
      console.error('BUG: Round did not end when all players finished guessing!');
      console.error('Expected: Round ends immediately when all 3 players have guessed');
      console.error('Actual: Round continues running even though all players finished');
      throw new Error('Round ending bug: Round did not end when all players guessed');
    }
  ...
  it('should prevent multiple guesses by checking onMapClick prop - EXPECTED TO FAIL', async () => {
    ...
    const afterFirstClickState = screen.getByTestId('click-enabled').textContent;
    ...
    for (let i = 2; i <= 5; i++) {
      ...
      if (currentGuessCount !== '1') {
        console.error(`❌ BUG: Multiple guessing allowed! Guess count increased to ${currentGuessCount}`);
        
        // Check what guesses are visible
        const humanGuesses = screen.queryAllByTestId(/^visible-guess-\d+$/)
        ...
        throw new Error(`Multiple guessing bug: Human player made ${humanGuesses.length} guesses, should only be 1`);
      }
    }
  ...
  it('should demonstrate the actual user experience bug', async () => {
    ...
    if (!roundResults) {
      ...
      console.log('5. User tries to make more guesses because round appears stuck...');
      ...
      if (finalGuessCount !== '3') {
        ...
        throw new Error(`User experience bugs confirmed: ${finalGuessCount} guesses instead of 3, round never ended`);
      }
    } else {
