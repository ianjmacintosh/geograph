...
describe('Scoring Behavior', () => {
      ...
      if (changes.length > 1) {
        ...
        for (let i = 1; i < changes.length; i++) {
          if (changes[i] === '0' && changes[i-1] !== '0') {
            throw new Error(`Score flickered for ${playerId}: ${changes[i-1]} -> ${changes[i]}`);
          }
        }
      }
    });
  ...
  it('should show correct score values after calculations are complete', async () => {
    ...
    const { calculatePlacementPoints, calculateBonusPoints } = await import('../utils/game');
    ...
    // Verify total points
    const player1Total = (player1Result?.placementPoints || 0) + calculateBonusPoints(0);
    const player2Total = (player2Result?.placementPoints || 0) + calculateBonusPoints(150);
    const player3Total = (player3Result?.placementPoints || 0) + calculateBonusPoints(300);
