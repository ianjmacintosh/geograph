...
vi.mock('../utils/game', () => ({
  calculateDistance: vi.fn((lat1: number, lng1: number, lat2: number, lng2: number) => {
    const distance = lat1 === 40.7128 && lng1 === -74.0060 ? 0 : (lat1 === 41.0 ? 150 : 300);
    console.log(`calculateDistance(${lat1}, ${lng1}, ${lat2}, ${lng2}) = ${distance}`);
    return distance;
  }),
  ...
  calculatePlacementPoints: vi.fn((guesses: Array<{playerId: string, distance: number}>, totalPlayers: number) => {
    console.log(`calculatePlacementPoints called with:`, guesses, `totalPlayers: ${totalPlayers}`);
  ...
  generateComputerGuess: vi.fn((city: any, accuracy: number) => {
    const guess = accuracy === 0.5 ? { lat: 41.0, lng: -73.0 } : { lat: 39.0, lng: -75.0 };
    console.log(`generateComputerGuess(${city.name}, ${accuracy}) = ${JSON.stringify(guess)}`);
    return guess;
  })
}));
  ...
  it('should debug the scoring calculation process', async () => {
    ...
    const trackScores = (event: string) => {
      try {
        ...
        const guessCount = mapElement ? parseInt(mapElement.textContent?.match(/\((\d+) guesses\)/)?.[1] || '0') : 0;
    ...
    scoreTracker.forEach(entry => {
      console.log(`${entry.event}: Guesses=${entry.guessCount}, Scores=${JSON.stringify(entry.scores)}`);
    });

    // Import the mocked functions to check call counts
    const { calculateDistance, calculateBonusPoints, calculatePlacementPoints, generateComputerGuess } = await import('../utils/game');
    
    // Print mock call logs
    console.log('\n=== MOCK FUNCTION CALLS ===');
    console.log('calculateDistance calls:', vi.mocked(calculateDistance).mock.calls.length);
    console.log('calculateBonusPoints calls:', vi.mocked(calculateBonusPoints).mock.calls.length);
    console.log('calculatePlacementPoints calls:', vi.mocked(calculatePlacementPoints).mock.calls.length);
    console.log('generateComputerGuess calls:', vi.mocked(generateComputerGuess).mock.calls.length);
    ...
    if (vi.mocked(calculatePlacementPoints).mock.calls.length === 0) {
      ...
      throw new Error('calculatePlacementPoints was never called - this is the root cause');
    }
    
    if (finalScores.player2 === '0' || finalScores.player3 === '0') {
      console.log('❌ ISSUE: Computer players have 0 scores after all processing');
      ...
      throw new Error(`Computer players missing scores: ${JSON.stringify(finalScores)}`);
    }
