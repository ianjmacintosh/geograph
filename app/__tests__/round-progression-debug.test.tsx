...
vi.mock('../components/WorldMap', () => ({
  WorldMap: ({ onMapClick, guesses }: any) => (
    <div data-testid="world-map">
      ...
      <div data-testid="map-clickable">{onMapClick ? 'clickable' : 'not-clickable'}</div>
    </div>
  )
}));
...
describe('Round Progression Debug', () => {
  ...
  it('should debug round progression issue', async () => {
    ...
    if (round2Clickable === 'clickable') {
      ...
      const afterRound2HumanGuess = screen.getByTestId('guess-count').textContent;
      ...
      if (round2FinalGuesses === '3') {
        ...
        console.error('❌ Round 2 issue - expected 3 guesses, got:', round2FinalGuesses);
      }
    } else {
