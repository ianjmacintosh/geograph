...
describe.skip("Round Completion Timing Bug", () => {
  ...
  it("should end round immediately when human guesses last (after all computers)", async () => {
    console.log(
      "=== Testing Round Ends Immediately When Last Player Guesses ===",
    );
    ...
    try {
      ...
      console.log(
        "⚠️  Computers did not guess automatically, testing with different scenario",
      );
      console.log(
        "Testing: human guesses when there are already existing guesses in round state",
      );
    ...
    console.log(
      "All players have guessed, checking for immediate round completion...",
    );
    ...
    if (actualWaitTime > 5000) {
      console.error(
        "❌ BUG CONFIRMED: Round waited too long to end after final guess!",
      );
      console.error(`Expected < 5000ms, got ${actualWaitTime}ms`);
      throw new Error(
        `Round completion too slow: ${actualWaitTime}ms (should be < 5000ms)`,
      );
    }
  ...
  it("should wait for timer if not all players have guessed", async () => {
    console.log(
      "=== Testing Timer Still Works When Not All Players Guessed ===",
    );
  ...
  it("should show countdown timer that decreases over time", async () => {
    ...
    if (
      actualDecrease < expectedDecrease - 1 ||
      actualDecrease > expectedDecrease + 1
    ) {
      console.error(
        `❌ Timer not counting down properly: decreased by ${actualDecrease}s, expected ~${expectedDecrease}s`,
      );
