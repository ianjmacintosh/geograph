...
function simulateReactStateUpdate(currentGuesses: any[], newGuess: any, totalPlayers: number) {
  ...
  if (updatedGuesses.length === totalPlayers) {
    const placements = calculatePlacementPoints(
      updatedGuesses.map(g => ({ playerId: g.playerId, distance: g.distance })),
      totalPlayers
    );
...
describe("React State Simulation - Sequential Updates", () => {
  it("should handle gradual player guess addition", () => {
    ...
    gameState.guesses = simulateReactStateUpdate(gameState.guesses, guess1, players.length);
    ...
    gameState.guesses = simulateReactStateUpdate(gameState.guesses, guess2, players.length);
    ...
    gameState.guesses = simulateReactStateUpdate(gameState.guesses, guess3, players.length);
  ...
  it("should handle rapid state updates", async () => {
    ...
    const stateUpdate2 = await simulateAsyncStateUpdate([...initialGuesses, createSimulationGuess("player2", 150)], 50);
