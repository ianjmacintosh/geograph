...
describe('React State Update Simulation', () => {
  it('should demonstrate the flickering issue and verify the fix', () => {
    ...
    const humanGuess: Guess = {
      playerId: 'human',
      lat: 40.7, lng: -74.0, distance: 80,
      placementPoints: 0, bonusPoints: calculateBonusPoints(80), totalPoints: 0, placement: 0,
      timestamp: Date.now()
    }
    
    currentRound = { ...currentRound, guesses: [humanGuess] }
    stateUpdates.push({ step: 'human-guess', guesses: [...currentRound.guesses] })

    // Step 2: Computer guesses are added (this was causing intermediate updates)
    const computerGuesses: Guess[] = [
      {
        playerId: 'comp1', lat: 40.8, lng: -74.1, distance: 200,
        placementPoints: 0, bonusPoints: calculateBonusPoints(200), totalPoints: 0, placement: 0,
        timestamp: Date.now()
      },
      {
        playerId: 'comp2', lat: 40.9, lng: -74.2, distance: 400,
        placementPoints: 0, bonusPoints: calculateBonusPoints(400), totalPoints: 0, placement: 0,
        timestamp: Date.now()
      }
    ]
    ...
    currentRound = { ...currentRound, guesses: [...currentRound.guesses, ...computerGuesses] }
    stateUpdates.push({ step: 'add-computer-guesses-no-placements', guesses: [...currentRound.guesses] })
    ...
    if (currentRound.guesses.length >= totalPlayers) {
      ...
      stateUpdates.push({ step: 'add-placements', guesses: [...currentRound.guesses] })
    }
    ...
    stateUpdatesFixed.push({ step: 'human-guess', guesses: [...fixedRound.guesses] })
    ...
    const guessDataFixed = allGuesses.map(g => ({ playerId: g.playerId, distance: g.distance }))
    const placementsFixed = calculatePlacementPoints(guessDataFixed, totalPlayers)
    
    // Update all guesses with placements in one go
    const finalGuesses = allGuesses.map(guess => {
      const placement = placementsFixed.find(p => p.playerId === guess.playerId)!
      return {
        ...guess,
        placementPoints: placement.placementPoints,
        placement: placement.placement,
    ...
    stateUpdatesFixed.push({ step: 'all-final-scores', guesses: [...fixedRound.guesses] })
    ...
    const intermediateUpdates = stateUpdatesFixed.filter(update => 
      update.step.includes('no-placements') || update.step.includes('add-computer-guesses')
    )
  ...
  it('should validate the exact timing of score visibility', () => {
    ...
    const singlePlayerState = { playersGuessed: 1, totalPlayers: 1, shouldShowScores: true }
    const singlePlayerReady = singlePlayerState.playersGuessed >= singlePlayerState.totalPlayers
